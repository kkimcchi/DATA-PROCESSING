"""
Random Forest 하이퍼파라미터 튜닝 실험 (런던, Train=2015 -> Test=2016)
- 목적: "RF를 튜닝하면 선형회귀보다 나아지는가?"를 실측으로 확인
- 방법: Train(2015) 안에서 TimeSeriesSplit(시간순 교차검증)으로 그리드서치 ->
  한 번도 보지 않은 Test(2016)에서 최종 평가
...
def run():
    ...
    # ---- 2) 그리드서치 튜닝 (Train 내부에서만 시간순 교차검증) ----
    param_grid = {
        "n_estimators": [300],
    ...
    evaluate("RF 튜닝됨 (2015->2015, 참고)", y_train, tuned.predict(X_train))
    ...
    evaluate("RF 강한 정규화 (2015->2015, 참고)", y_train, strong.predict(X_train))
