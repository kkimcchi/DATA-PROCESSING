"""
정규화 선형회귀(Ridge, Lasso) 하이퍼파라미터 튜닝 실험 (런던, Train=2015 -> Test=2016)
...
- CV는 Train(2015) 내부에서만 수행, Test(2016)는 최종 평가에만 사용(데이터 누수 방지)
"""
import numpy as np
...
def tune_and_evaluate(name, estimator_cls, X_train, y_train, X_test, y_test):
    ...
    evaluate(f"{name} 튜닝됨 (2015->2016)", y_test, best.predict(X_test))
    evaluate(f"{name} 튜닝됨 (2015->2015, 참고)", y_train, best.predict(X_train))
    return best
